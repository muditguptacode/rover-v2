const setEye = function (message) {
    ...
    const scaledLinearMagnitude = scaleNumber(Math.abs(x), 0, 1, 0.7, 1.5);
    ...
    eye.css({
        ...
        'transform': 'rotate(' + rot + 'deg) scale(' + scaledLinearMagnitude + ')'
    });
}

setBattery = function (message) {
    const { voltage } = message;
    document.getElementsByClassName("battery")[0].innerHTML = `${voltage / 10} V`;
}
